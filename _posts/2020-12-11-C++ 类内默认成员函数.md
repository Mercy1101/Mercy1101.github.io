...
原因：
一个成员有删除的或不可访问的析构函数会导致合成的默认和拷贝构造函数被定义为删除的，为了防止创建出无法销毁的对象。
...
```
如果一个类有一个拷贝构造函数但未定义移动构造函数时，编译器不会合成移动构造函数。
如果一个类没有移动构造函数，函数匹配规则保证该类型的对象会被拷贝，即使我们试图使用`std::move`来移动他们时也是如此。
